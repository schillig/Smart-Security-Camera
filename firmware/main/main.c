...
// -------------------------------------------------------------
// HTTP API: Endpoint 1 (/stream) - Port 81
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// HTTP API: Endpoint 2 (/control) - Port 80
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// HTTP API: Endpoint 3 (/update) - Port 80 (OTA)
// -------------------------------------------------------------
static esp_err_t ota_update_handler(httpd_req_t *req) {
    ...
    while (remaining > 0) {
        ...
        if (received <= 0) {
            ...
            ESP_LOGE(TAG, "Connection lost during OTA!");
            httpd_resp_send_500(req);
            return ESP_FAIL;
        }
        esp_ota_write(ota_handle, (const void *)buf, received);
        remaining -= received;
    }
    ...
    ESP_LOGE(TAG, "OTA failed at the very end.");
    httpd_resp_send_500(req);
    return ESP_FAIL;
}
...
void app_main(void) {
    ...
    if (httpd_start(&api_server, &config_api) == ESP_OK) {
        ...
        httpd_register_uri_handler(api_server, &ota_uri);
        ESP_LOGI(TAG, "Control & OTA API started on Port 80");
    }
    ...
    if (httpd_start(&stream_server, &config_stream) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "Video Stream started on Port 81 - VERSION 2 WIRELESS!");
    }
}
